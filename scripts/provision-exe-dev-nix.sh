...
# - trigger first devenv activation in the toolnix checkout
#
...
Minimal host-native provisioner for exe.dev VMs.
...
log "Writing ~/.zshenv for nix-daemon"
...
ln -sf "$HOME/.env.toolnix" "$HOME/.env.toolbox"
...
chmod +x "$TOOLNIX_DIR"/scripts/*.sh 2>/dev/null || true

log "Triggering first devenv activation in toolnix"
cd "$TOOLNIX_DIR"
direnv allow .
direnv exec . true
direnv exec . bash "$TOOLNIX_DIR/scripts/toolnix-setup-hook.sh" true

log "Minimal Nix/devenv provisioning complete"
REMOTE_SCRIPT
...
ssh "${SSH_OPTS[@]}" "$TARGET_FQDN" \
  "bash '$REMOTE_TMP/remote-host-native-setup.sh' '$REMOTE_TMP' '$REMOTE_REPO_DIR' '$MAIN_REPO_BRANCH' '$REPO_URL'"

REMOTE_TOOLNIX_DIR="/home/exedev/sources/toolnix"
run_smoke_tests "$TARGET_FQDN" "$TARGET_MODE" "$REMOTE_TOOLNIX_DIR" "$SSH_OPTS_STR"
